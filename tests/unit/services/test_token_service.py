...
from src.auth.dto import BaseUserDTO
...
async def test_generate_access_token_structure(token_service):
    ...
    user_dto = BaseUserDTO(id=123, name="Test", login="test", email="t@t.com")
...
async def test_decode_valid_token(token_service):
    ...
    user_dto = BaseUserDTO(id=1, name="A", login="a", email="a@a.com")
    token = await token_service.generate_access_token(user_dto)

    payload = await token_service.decode_token(token.token)
...
async def test_verify_refresh_token_logic(token_service):
    ...
    with pytest.raises(InvalidTokenError) as exc:
        await token_service.verify_refresh_token(access_token.token)
